...
check_system(){
    ...
    if [[ "${ID}" == "debian" && ${VERSION_ID} -ge 8 ]];then
        ...
        echo -e "${OK} ${Blue} 当前系统为 Ubuntu ${VERSION_ID} ${Font} "
    else
...
aria2ng_install(){
	...
	if [[ $? -eq 0 ]];then
        echo -e "${OK} ${Blue} AriaNg 下载成功 ${Font}"
        sleep 1
    else
...
echo -e "${Blue} 请选择VIM编辑后输入:wq保存 ${Font}"
crontab -e
